...
@pytest.mark.asyncio
async def test_user_rate_post_not_found(db):	async def test_user_rate_post_not_found(db):
    result = await ratings.user_rate_post(user_id=1, post_id=10, db=db, user=None)	    result = await ratings.user_rate_post(user_id=1, post_id=10, db=db, user=None)
    assert result is None	    assert result is None
 193 changes: 193 additions & 0 deletions193  
tests/test_rating_repository.py
